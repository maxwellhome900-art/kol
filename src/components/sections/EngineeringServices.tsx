...
import { engineeringServices, site } from "@/lib/data";
      ...
      <div className="mx-auto max-w-6xl">
        ...
        >
          ...
          <h2 className="mt-3 font-[family-name:var(--font-syne)] text-3xl font-semibold tracking-tight text-[var(--text-primary)] md:text-4xl">
            How we work — {site.name}
          </h2>
          <p className="mt-4 text-[var(--text-muted)]">
            Parallel to the photography packages: clear scope, a price band, and{" "}
            <button
              type="button"
              onClick={() => openMarkoAI()}
            ...
            to route the first conversation. Photography holds live on the
            studio site; engineering retainers are scoped in contact.
          </p>
        </motion.div>

        <div className="grid gap-6 md:grid-cols-3">
          {engineeringServices.map((tier, index) => (
            <motion.article
              key={tier.name}
              ...
              className={`relative flex flex-col rounded-3xl border p-6 md:p-8 ${
                tier.highlighted
                  ? "border-sky-400/45 bg-gradient-to-b from-sky-500/15 to-transparent shadow-lg shadow-sky-500/10"
                  : "border-[var(--border-glass)] glass"
              }`}
            >
              {tier.highlighted && (
                <span className="mb-4 inline-flex w-fit rounded-full bg-sky-400/25 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-sky-100">
                  Signature
                </span>
              )}
              ...
              <p className="mt-6 font-[family-name:var(--font-syne)] text-3xl font-bold text-[var(--text-primary)]">
                {tier.price}
                {tier.unit ? (
